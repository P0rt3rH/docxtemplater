...
TAG`;
	...
	describe("Raw Xml Insertion", function () {
		it("should work with simple example", function () {
			var inner = '<w:t>{@complexXml}</w:t>'
			var content = `<w:p><w:r>${inner}</w:r></w:p>`;
...
<w:t>}{/body}</w:t>`;
		...
		it("replaces the xml tag no problem", function () {
			var content = "<w:p><w:t>{@myra</w:t><w:t>wtag}</w:t><w:t>slfkdsjfh</w:t><w:t>{@myra</w:t><w:t>wtag}</w:t></w:p>";
			var scope = {
				myrawtag: "<w:p><w:t>foobar</w:t></w:p>",
				myrawtag2: "<w:p><w:t>fizzbaz</w:t></w:p>"
			};
			var xmlTemplater = new XmlTemplater(content, {fileTypeConfig: FileTypeConfig.docx, tags: scope});

			xmlTemplater.render();
			expect(xmlTemplater.getFullText()).to.eql("foobarslfkdsjfhfoobar");

		});

		it("should work when rawtag not in paragraph", function () {
			var content = "<w:p><w:t>{@myrawtag}</w:t></w:p>";
			var scope = { myrawtag: "<w:r><w:t>foobar</w:t></w:r>" };
			var xmlTemplater = new XmlTemplater(content, { fileTypeConfig: FileTypeConfig.docx, tags: scope });

			xmlTemplater.render();
			expect(xmlTemplater.content).to.eql("<w:p><w:r><w:t>foobar</w:t></w:r></w:p>");
		});
	});
